...
public:
    ...
    bool save();
    bool saveAs();
    bool canClose();  //judge if the file may be closed
    ...
    //emit highlight() signal in the function
    ...
    Q_INVOKABLE void showPrevious(); //highlight previous result circularly
    Q_INVOKABLE void showNext();     //highlight next result circularly
...
private:
    ...
    void addCommand(std::unique_ptr<EditCommand> &&command);//add a command to 'historyList'
    void highlightAll(int length = 1);  //highlight all search result
